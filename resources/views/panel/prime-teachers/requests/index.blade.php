                ...
                <div class="table-responsive">
                    <table class="table table-bordered card-table table-striped table-vcenter text-nowrap mb-0"
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <a href="{{route('panel.teacher.edit',$request->teacher->id)}}">
                                        {{$request->teacher->first_name}} {{$request->teacher->last_name}}
                                    </a>
                                </td>
                                <td>
                                    <a href="{{route('panel.prime.teacher.requests.show',$request->id)}}" class="btn btn-warning">نمایش فایل ها</a>
                                </td>
                                <td>
                                    @if ($request->status)
                                ...
                                <td class="d-flex gap-2">
                                    <a href="{{route('panel.prime.teacher.requests.edit',$request->id)}}" class="btn btn-sm btn-info">
                                        <i class="fe fe-edit-2"></i>
                                    </a>
                                    <form action="{{route('panel.prime.teacher.requests.delete')}}" method="POST"
