...
<div class="page-header">
    <div>
        <h2 class="main-content-title tx-24 mg-b-5">لیست پیام ها</h2>
        <ol class="breadcrumb">
    ...
    <div class="d-flex">
        <div class="justify-content-center">
            <a href="{{route('panel.ticket.index')}}" type="button"
    ...
    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 grid-margin">
        <div class="card custom-card">
            ...
            <div class="card-body" style="height: 500px; overflow-y: scroll;background-color: rgb(245, 245, 245)">
                @foreach ($ticketContents as $content)
                <div class="mt-5 border border-b p-3 rounded-3 bg-white">
                    @if ($content->sender != 'admin')
                    <a href="{{route('panel.users.edit',$content->ticket->user->id)}}">
                        <p>{{$content->ticket->user->first_name}} {{$content->ticket->user->last_name}}</p>
                    </a>
                    @else
                    ...
                    <p>{{$content->text}}</p>
                    <hr>
                    @if ($content->file != NULL && is_array(json_decode($content->file)))

                    <a href="{{json_decode($content->file)[0]}}" class="btn btn-info">فایل ضمیمه</a>
                    @endif
                </div>
                @endforeach
            </div>
            ...
            <div class="border border-t">
                <form class="py-3" action="{{route('panel.ticket.content.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf
