            ...
            <div class="card-body">
                <div class="table-responsive border userlist-table">
                    ...
                    <table class="table card-table table-striped table-vcenter text-nowrap mb-0">
                        <thead>
                            <tr>
                                <th class="wd-lg-20p"><span> حداقل سن</span></th>
                                <th class="wd-lg-20p"><span> حداکثر سن</span></th>
                                <th class="wd-lg-20p"><span> جنسیت</span></th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($jobRequirements as $key => $jobRequirement)
                            <tr>
                                <td>{{$jobRequirement->min_age}}</td>
                                <td>{{$jobRequirement->max_age}}</td>
                                <td>{{$jobRequirement->sex}}</td>
                                
                                <td class="d-flex gap-2">
                                    <a href="{{route('panel.advertisement.job.requirements.edit',[$advertisement->id,$jobRequirement->id])}}"
                                        class="btn btn-sm btn-info">
                                        <i class="fe fe-edit-2"></i>
                                    </a>
                                    <form action="{{route('panel.advertisement.job.requirements.delete')}}" method="POST"
                                        onsubmit="return confirm('آیا مطمئن هستید؟')">
                                        @csrf
                                        <input type="hidden" value="{{$jobRequirement->id}}" name="id">
                    ...
                    <div class="text-center p-5">
                        ...
                        <a href="{{route('panel.advertisement.job.requirements.create',$advertisement->id)}}" type="button"
