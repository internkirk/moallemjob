...
class UploadVideoController extends Controller
{
    ...
    public function index($id)
    {
        ...
        return view('panel.shop.course.upload.index',compact('episode'));
    }
    ...
    public function store(Request $request)
    {
        ...
        Storage::disk('private')->deleteDirectory('videos/'.$request->episodeId);
        ...
        if (count($chunksUploaded) == $totalChunks) {
            ...
            // $finalDir =  Storage::disk('local')->makeDirectory('/videos/'.$request->episodeId);
            $finalDir = storage_path('app/private/videos/'.$request->episodeId);
            ...
            CourseEpisode::findOrFail($request->episodeId)->update([
                'link' => '/storage'.$finalPath[1]
            ]);
        }
